 ...
 */
public class PickUp implements JavaFunction {
        ...
        if(game.getDuck().getHolding() == null) {
            for(GameObject gameObject : game.getGameObjects()) {
                if(gameObject.getXInGrid() == x && gameObject.getYInGrid() == y && !gameObject.equals(game.getDuck()) && gameObject instanceof Box) {
                    game.getDuck().setHolding((Box) gameObject);
                    break;
                }
            }
            if(game.getDuck().getHolding() != null) {
                game.getGameObjects().remove(game.getDuck().getHolding());
            }
        }
        
        game.tick();
        
        return new Variable(VariableType.BOOLEAN,game.getDuck().getHolding() != null);
    }

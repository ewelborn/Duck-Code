 ...
 */
public class ScanForward implements JavaFunction {

    private final Game game;

    public ScanForward(Game game) {
        this.game = game;
    }
    ...
    public Variable execute(List<Variable> args) {
        ...
        for(GameObject gameObject : game.getGameObjects()) {
            if(gameObject.getXInGrid() == x && gameObject.getYInGrid() == y && !gameObject.equals(game.getDuck())) {
                result = gameObject;
            }
        }
        
        if(result != null) {
            return new Variable(VariableType.STRING,result.toString());
        } else {
            return new Variable(VariableType.STRING,"none");
        }
    }
}
